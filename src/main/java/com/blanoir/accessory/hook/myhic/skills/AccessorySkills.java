...
public final class AccessorySkills {
    ...
    public void loadConfig() {
        ...
        List<YamlConfiguration> skillConfigs = ConfigFiles.yamlFiles(new File(plugin.getDataFolder(), "skill"))
                .stream().map(file -> ConfigFiles.load(file, plugin.getLogger())).filter(Objects::nonNull).toList();
        int signature = 1;
    ...
    private void debug(String message) {
        if (!plugin.settings().skillDebug()) return;
